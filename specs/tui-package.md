...
`packages/agent/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/agent`,
`packages/cli`, or `@hannah/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/agent`.
- Do not replace private `packages/agent` imports with `@hannah/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@hannah/tui`.
...
```text
packages/agent/src/cli/cmd/tui
```

Its private dependency on `packages/agent` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/agent/src/*`.
...
Status: Completed. The private `@hannah/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@hannah/tui`.
...
- The package has no dependency on `opencode`, `@hannah/cli`, or
  `@hannah/core`.
...
primitives now live in `@hannah/tui`. The legacy host consumes explicit
...
and build inputs through `@hannah/tui/runtime`. Movable app, component,
...
`@hannah/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/agent` initially.
...
canonical in `@hannah/tui`. Configured references resolve through the new
...
`@hannah/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/agent`.
...
- The package has no imports from `packages/agent`, `packages/cli`, or
  `@hannah/core`.
...
public `@hannah/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/agent`.
- Keep the legacy embedded worker and server startup in `packages/agent`.
- Change those adapters to load config, create transport inputs, and call the
  public `@hannah/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/agent`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@hannah/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@hannah/core`, or either
  executable package.
...
rg '@hannah/core|packages/agent|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/agent/src packages/agent/test
```
...
- Build the current-platform `packages/agent` binary.
