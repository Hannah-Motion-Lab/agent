...
import { PromptInput } from "@hannah/schema/prompt-input"
import { Session } from "@hannah/schema/session"
import { Project } from "@hannah/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@hannah/schema/schema"
...
import { SessionEvent } from "@hannah/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Hannah process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Hannah process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
