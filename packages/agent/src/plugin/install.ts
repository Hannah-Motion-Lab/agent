...
export type PatchDeps = {
  ...
  files: (dir: string, name: "hannah-agent" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".hannah-agent")
}

function patchName(kind: Kind): "hannah-agent" | "tui" {
  if (kind === "server") return "hannah-agent"
  return "tui"
}
