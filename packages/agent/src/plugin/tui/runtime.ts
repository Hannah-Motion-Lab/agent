...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@hannah/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@hannah/core/effect/app-node-builder"
import { errorData, errorMessage } from "@hannah/tui/util/error"
...
import { hasTheme, upsertTheme } from "@hannah/tui/context/theme"
import { Global } from "@hannah/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@hannah/core/util/flock"
import { Flag } from "@hannah/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@hannah/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@hannah/core/v1/config/plugin"
import { createCommandShim } from "@hannah/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@hannah/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".hannah-agent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".hannah-agent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".hannah-agent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.HANNAH_AGENT_PURE ? [] : pluginOrigins
    if (Flag.HANNAH_AGENT_PURE && pluginOrigins.length) {
    }
