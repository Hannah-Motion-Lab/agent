...
export async function checkPluginCompatibility(target: string, engineVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(engineVersion) || semver.major(engineVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["hannah-agent"]
  if (typeof range !== "string") return
  if (!semver.satisfies(engineVersion, range)) {
    throw new Error(`Plugin requires hannah-agent ${range} but running ${engineVersion}`)
  }
}
