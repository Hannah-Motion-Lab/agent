...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  for (const [key, entry] of Object.entries(yield* mcp.tools())) {
    ...
    // Which server this tool came from. Hannah's policy layer allowlists MCP by
    // *server*, and without this it would have to guess one from the tool key —
    // `playwright_browser_navigate` could be either server `playwright` or
    // server `playwright_browser`. See docs/decisions/0012.
    const server = entry.clientName
      ...
      run.promise(
        Effect.gen(function* () {
          ...
          const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* Effect.gen(function* () {
            yield* ctx.ask({ permission: key, metadata: { mcp: server }, patterns: ["*"], always: ["*"] })
