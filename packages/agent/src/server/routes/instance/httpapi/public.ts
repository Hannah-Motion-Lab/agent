...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for hannah-agent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(HannahHttpApi)
...
export const PublicApi = HannahHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "hannah-agent",
    version: "1.0.0",
    description: "hannah-agent engine api",
