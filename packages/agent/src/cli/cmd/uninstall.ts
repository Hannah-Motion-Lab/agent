...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall hannah-agent and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Hannah")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g hannah-agent-ai",
      pnpm: "pnpm uninstall -g hannah-agent-ai",
      bun: "bun remove -g hannah-agent-ai",
      yarn: "yarn global remove hannah-agent-ai",
      brew: "brew uninstall hannah-agent",
      choco: "choco uninstall hannah-agent",
      scoop: "scoop uninstall hannah-agent",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".hannah-agent")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Hannah!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# hannah-agent") || content.includes(".hannah-agent/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".hannah-agent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".hannah-agent/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".hannah-agent"))
    ) {
      continue
    }
