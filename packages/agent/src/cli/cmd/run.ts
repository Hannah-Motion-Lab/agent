import type { PermissionV1 } from "@hannah/core/v1/permission"
import { FSUtil } from "@hannah/core/fs-util"
// CLI entry point for `hannah-agent run` and `hannah-agent --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`hannah-agent --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`hannah-agent --mini --attach`): connects to a running
//      hannah-agent server and runs interactive mode against it.
//
...
import { createHannahClient, type HannahClient, type ToolPart } from "@hannah/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<HannahClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run hannah-agent with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running hannah-agent server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to HANNAH_AGENT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to HANNAH_AGENT_SERVER_USERNAME or 'hannah-agent')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: HannahClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: HannahClient, sessionID: string) {
      ...
      async function current(sdk: HannahClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: HannahClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: HannahClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: HannahClient) {
        ...
        async function loop(client: HannahClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createHannahClient({
        baseUrl: "http://hannah-agent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
