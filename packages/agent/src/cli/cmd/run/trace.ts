...
//
// Enable with HANNAH_AGENT_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/hannah-agent/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.HANNAH_AGENT_DIRECT_TRACE) {
    state = false
    return undefined
  }
