// Shared type vocabulary for the direct interactive mode (`hannah-agent --mini`).
//
...
import type { HannahClient, PermissionRequest, QuestionRequest, ToolPart } from "@hannah/sdk/v2"
import type { TuiConfig } from "@hannah/tui/config"
...
type PromptModel = Parameters<HannahClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<HannahClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<HannahClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<HannahClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<HannahClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<HannahClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<HannahClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<HannahClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<HannahClient["question"]["reject"]>[0]
