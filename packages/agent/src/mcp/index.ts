...
import { ConfigMCPV1 } from "@hannah/core/v1/config/mcp"
...
import { McpEvent } from "@hannah/schema/mcp-event"
...
function createClient(directory: string) {
  const client = new Client({ name: "hannah-agent", version: InstallationVersion }, CLIENT_OPTIONS)
...
export interface McpTool {
  ...
  /**
   * The server this tool came from. The record key mangles both names together
   * (`playwright_browser_navigate`), so it cannot be split back apart — and
   * Hannah's policy allowlists MCP by server (docs/decisions/0012).
   */
...
export class Service extends Context.Service<Service, Interface>()("@hannah/MCP") {}
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        for (const def of listed) {
          result[McpCatalog.toolName(clientName, def.name)] = { def, client, clientName, timeout }
        }
      }
      return result
    })
