...
import type { AssistantMessage, Message, HannahClient, SessionMessageResponse } from "@hannah/sdk/v2"
...
export const AuthMethodID = "hannah-agent-login"
...
export class Service extends Context.Service<Service, Interface>()("@hannah-agent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `hannah-agent auth login` in the terminal",
      name: "Login with hannah-agent",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: HannahClient) {
...
function makeUsageService(sdk: HannahClient) {
...
async function loadDirectorySnapshot(sdk: HannahClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, hannah-agent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  // "opencode" here is the upstream catalog's gateway provider id, not branding.
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Hannah service failure", service })
}
