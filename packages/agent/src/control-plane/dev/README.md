This is a plugin to simulate a remote environment locally. Add this to `.hannah-agent/hannah-agent.jsonc`:

```json
  "plugin": ["../packages/agent/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/agent/script/run-workspace-server
```
