...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from hannah-agent.json files
...
async function configFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "hannah-agent"),
    ...(await Filesystem.findUp(["hannah-agent.json", "hannah-agent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "hannah-agent"))
  }
  if (Flag.HANNAH_AGENT_CONFIG) files.push(Flag.HANNAH_AGENT_CONFIG)
