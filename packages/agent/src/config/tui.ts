...
import { isRecord } from "@hannah/tui/util/record"
...
export class Service extends Context.Service<Service, Interface>()("@hannah/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old hannah-agent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.hannah-agent`
  // folders between cwd and home, and HANNAH_AGENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.HANNAH_AGENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit HANNAH_AGENT_TUI_CONFIG override, if set.
  if (Flag.HANNAH_AGENT_TUI_CONFIG) {
    const configFile = Flag.HANNAH_AGENT_TUI_CONFIG
  ...
  // 4. `.hannah-agent` directories (and HANNAH_AGENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".hannah-agent") || dir === Flag.HANNAH_AGENT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".hannah-agent") && dir !== Flag.HANNAH_AGENT_CONFIG_DIR) continue
