...
- Author package entrypoint is `@hannah/plugin/tui`.
...
- Import types from `@hannah/plugin/tui`.
- `@hannah/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@hannah/plugin/tui"
...
- If `engines.hannah-agent` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.hannah-agent` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.hannah-agent`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.hannah-agent`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.hannah-agent` dir or `HANNAH_AGENT_CONFIG_DIR`, OpenCode installs `@hannah/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@hannah/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@hannah/plugin/tui`.
...
- Local plugins persist installed themes under the local `.hannah-agent/themes` area near the plugin config source.
...
- `--pure` / `HANNAH_AGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.hannah-agent/plugins/tui-smoke.tsx`
- Local vim plugin: `.hannah-agent/plugins/tui-vim.tsx`
- Local smoke config: `.hannah-agent/tui.json`
- Local smoke theme: `.hannah-agent/plugins/smoke-theme.json`
