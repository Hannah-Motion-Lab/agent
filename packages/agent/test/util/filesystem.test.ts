...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "hannah-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "hannah-agent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hannah-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "hannah-agent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["hannah-agent.json", "hannah-agent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "hannah-agent.json"),
        path.join(project, "hannah-agent.jsonc"),
      ])
    })
  })
