...
it.instance("plan agent denies edits except .hannah-agent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".hannah-agent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".hannah-agent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.HANNAH_AGENT_TEST_HOME
      process.env.HANNAH_AGENT_TEST_HOME = test.directory
