...
import { HannahClient, type GlobalEvent } from "@hannah/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<HannahClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<HannahClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<HannahClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<HannahClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<HannahClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: HannahClient["event"]["subscribe"]
    globalEvent?: HannahClient["global"]["event"]
    promptAsync?: HannahClient["session"]["promptAsync"]
    status?: HannahClient["session"]["status"]
    messages?: HannahClient["session"]["messages"]
    children?: HannahClient["session"]["children"]
    permissions?: HannahClient["permission"]["list"]
    questions?: HannahClient["question"]["list"]
  } = {},
) {
  const client = new HannahClient()

  const subscribe: HannahClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: HannahClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: HannahClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: HannahClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: HannahClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: HannahClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: HannahClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: HannahClient["question"]["list"] = input.questions ?? (() => ok([]))
