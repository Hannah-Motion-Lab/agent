...
import type { QuestionRequest } from "@hannah/sdk/v2"
import { HannahKeymapProvider, registerHannahKeymap } from "@hannah/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerHannahKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerHannahKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerHannahKeymap(keymap, renderer, tuiConfig)
