...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/hannah-agent.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/hannah-agent.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/hannah-agent.jsonc", dir: ".hannah-agent", suggestion: "opencode" },
        expected:
          'Directory ".hannah-agent" in /tmp/hannah-agent.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/hannah-agent.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/hannah-agent.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `hannah-agent models` to list available models",
      "Or check your config (hannah-agent.json) provider/model names",
    ].join("\n")
