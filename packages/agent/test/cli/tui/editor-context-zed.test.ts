...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@hannah/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.HANNAH_AGENT_ZED_DB
  process.env.HANNAH_AGENT_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.HANNAH_AGENT_ZED_DB
    else process.env.HANNAH_AGENT_ZED_DB = previous
    home.mockRestore()
  }
})
