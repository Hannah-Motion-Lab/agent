...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const markerFile = path.join(tmp, ".git", "hannah-agent")
      expect(yield* Effect.promise(() => Bun.file(markerFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "hannah-agent")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "hannah-agent")
      const wrongCache = path.join(parentDir, ".git", "hannah-agent")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "hannah-agent")
      const cacheB = path.join(bareB, "hannah-agent")
      const wrongCache = path.join(parentDir, ".git", "hannah-agent")
