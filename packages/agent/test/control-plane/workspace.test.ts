...
import { EventSequenceTable } from "@hannah/core/event/sql"
...
import { WorkspaceV2 } from "@hannah/core/workspace"
import { WorkspaceTable } from "@hannah/core/control-plane/workspace.sql"
...
const originalEnv = {
  HANNAH_AGENT_AUTH_CONTENT: process.env.HANNAH_AGENT_AUTH_CONTENT,
  HANNAH_AGENT_EXPERIMENTAL_WORKSPACES: process.env.HANNAH_AGENT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.HANNAH_AGENT_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.HANNAH_AGENT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.HANNAH_AGENT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.HANNAH_AGENT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.HANNAH_AGENT_EXPERIMENTAL_WORKSPACES).toBe("true")
