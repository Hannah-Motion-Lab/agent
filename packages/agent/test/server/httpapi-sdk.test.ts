...
import { createHannahClient } from "@hannah/sdk/v2"
...
type Sdk = ReturnType<typeof createHannahClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.HANNAH_AGENT_SERVER_PASSWORD = input?.password
      Flag.HANNAH_AGENT_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".hannah-agent", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        // Contains, not equals: the fixture's own config file also scores against
        // this query, and this test is about location/workspace routing below.
        expect(found.data?.data).toContainEqual(expect.objectContaining({ path: "hello.txt", type: "file" }))
        ...
        expect(request!.headers.has("x-hannah-agent-directory")).toBe(false)
        expect(request!.headers.has("x-hannah-agent-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("hannah-agent", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("hannah-agent", "secret") },
        })
