    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-hannah-agent-directory": "/secret/path",
      ...
      expect(forwarded?.headers["x-hannah-agent-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-hannah-agent-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-hannah-agent-directory", headerDir),
        HttpClient.execute,
      )
