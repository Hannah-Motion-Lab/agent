...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    Flag.HANNAH_AGENT_SERVER_PASSWORD = "secret"
...
describe("HttpApi CORS", () => {
  ...
  // Upstream allowed any `*.opencode.ai` origin by default. This fork trusts
  // only loopback plus configured origins, so the assertion is now that an
  // arbitrary remote origin is *not* reflected back.
  it.live("adds CORS headers to unauthorized responses", () =>
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(
        HttpApiApp.createRoutes().pipe(
          Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ HANNAH_AGENT_SERVER_PASSWORD: "secret" }))),
        ),
        { disableLogger: true },
      ).handler
  ...
  it.live("does not trust upstream's domains", () =>
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(HttpApiApp.createRoutes(), { disableLogger: true }).handler
      const response = yield* Effect.promise(() =>
        handler(
          new Request(new URL("/global/config", "http://localhost"), {
      ...
      // A third-party origin must never be reflected: doing so would let a page
      // on that domain drive the agent's tool surface from the user's browser.
      expect(response.headers.get("access-control-allow-origin")).not.toBe("https://app.opencode.ai")
    }),
  )
