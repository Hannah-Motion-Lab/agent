 ...
 *
 * The script intentionally isolates `HANNAH_AGENT_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "hannah-agent.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "hannah-agent.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-hannah-agent-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-hannah-agent-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-hannah-agent-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
