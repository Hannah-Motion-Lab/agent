...
const original = {
  ...
  envPassword: process.env.HANNAH_AGENT_SERVER_PASSWORD,
  envUsername: process.env.HANNAH_AGENT_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.HANNAH_AGENT_SERVER_PASSWORD = original.HANNAH_AGENT_SERVER_PASSWORD
  Flag.HANNAH_AGENT_SERVER_USERNAME = original.HANNAH_AGENT_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.HANNAH_AGENT_SERVER_PASSWORD
  else process.env.HANNAH_AGENT_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.HANNAH_AGENT_SERVER_USERNAME
  else process.env.HANNAH_AGENT_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.HANNAH_AGENT_SERVER_PASSWORD = auth.password
  Flag.HANNAH_AGENT_SERVER_USERNAME = auth.username
  process.env.HANNAH_AGENT_SERVER_PASSWORD = auth.password
  process.env.HANNAH_AGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.HANNAH_AGENT_SERVER_PASSWORD = undefined
  Flag.HANNAH_AGENT_SERVER_USERNAME = auth.username
  delete process.env.HANNAH_AGENT_SERVER_PASSWORD
  process.env.HANNAH_AGENT_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-hannah-agent-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-hannah-agent-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-hannah-agent-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "hannah-agent.json"),
          JSON.stringify({ formatter: false, lsp: false, plugin: [pathToFileURL(plugin).href] }),
        )
        return { initialized, completed }
      },
    })
    const previous = process.env.HANNAH_AGENT_DISABLE_DEFAULT_PLUGINS
    process.env.HANNAH_AGENT_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.HANNAH_AGENT_DISABLE_DEFAULT_PLUGINS
      else process.env.HANNAH_AGENT_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-hannah-agent-ticket": "1" },
      })
