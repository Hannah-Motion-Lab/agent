...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-hannah-agent-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-hannah-agent-workspace": "internal",
          },
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-hannah-agent-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-hannah-agent-workspace")
