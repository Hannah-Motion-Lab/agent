...
import { PluginHost } from "@hannah/core/plugin/host"
import { OpencodePlugin } from "@hannah/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  ...
  it.live("loads providers and models from the connected Hannah server", () =>
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ HANNAH_AGENT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses HANNAH_AGENT_API_KEY as credentials", () =>
    withEnv({ HANNAH_AGENT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ HANNAH_AGENT_API_KEY: undefined, CUSTOM_HANNAH_AGENT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_HANNAH_AGENT_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
