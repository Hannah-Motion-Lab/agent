...
import { ModelsDevPlugin } from "@hannah/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.HANNAH_AGENT_MODELS_PATH,
          disabled: Flag.HANNAH_AGENT_DISABLE_MODELS_FETCH,
        }
        Flag.HANNAH_AGENT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.HANNAH_AGENT_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.HANNAH_AGENT_MODELS_PATH = previous.path
          Flag.HANNAH_AGENT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
