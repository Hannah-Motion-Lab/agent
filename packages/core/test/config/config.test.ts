...
import { Config } from "@hannah/core/config"
import { ConfigProvider } from "@hannah/core/config/provider"
import { AppNodeBuilder } from "@hannah/core/effect/app-node-builder"
import { LayerNode } from "@hannah/core/effect/layer-node"
import { ConfigMigrateV1 } from "@hannah/core/v1/config/migrate"
...
import { Policy } from "@hannah/core/policy"
import { Project } from "@hannah/core/project"
import { AbsolutePath } from "@hannah/core/schema"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "hannah-agent.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "hannah-agent.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "hannah-agent.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "hannah-agent.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "hannah-agent.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "hannah-agent.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "hannah-agent.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .hannah-agent configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".hannah-agent"), { recursive: true })
            await fs.mkdir(path.join(directory, ".hannah-agent"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "hannah-agent.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "hannah-agent.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "hannah-agent.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "hannah-agent.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "hannah-agent.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".hannah-agent", "hannah-agent.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".hannah-agent", "hannah-agent.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".hannah-agent")),
              AbsolutePath.make(path.join(directory, ".hannah-agent")),
            ])
