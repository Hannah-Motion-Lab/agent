...
import { define } from "@hannah/plugin/v2/effect"
...
import { LocationServiceMap } from "@hannah/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "hannah-agent.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "hannah-agent.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
