...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@hannah/acme")).toBe("@hannah/acme")
    expect(Npm.sanitize("@hannah/acme@1.0.0")).toBe("@hannah/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/hannah/acme.git"
    const expected = win ? "acme@git+https_//github.com/hannah/acme.git" : spec
