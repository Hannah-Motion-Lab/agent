...
export function fileLogger(file = path.join(Global.Path.log, "hannah-agent.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.HANNAH_AGENT_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.HANNAH_AGENT_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
