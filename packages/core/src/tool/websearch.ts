...
import { ToolFailure } from "@hannah/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@hannah/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.HANNAH_AGENT_WEBSEARCH_PROVIDER === "exa" || process.env.HANNAH_AGENT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.HANNAH_AGENT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("HANNAH_AGENT_EXPERIMENTAL") || truthy("HANNAH_AGENT_ENABLE_EXA") || truthy("HANNAH_AGENT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("HANNAH_AGENT_ENABLE_PARALLEL") || truthy("HANNAH_AGENT_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `hannah-agent/${InstallationVersion}`,
