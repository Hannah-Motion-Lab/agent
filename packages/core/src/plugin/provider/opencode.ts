...
import type { IntegrationOAuthMethodRegistration } from "@hannah/plugin/v2/effect/integration"
import { define } from "@hannah/plugin/v2/effect/plugin"
import type { CredentialValue } from "@hannah/sdk/v2/types"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.HANNAH_AGENT_API_KEY || connected || item.provider.request.body.apiKey)
