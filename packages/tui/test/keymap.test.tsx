...
import { getHannahModeStack, HANNAH_AGENT_BASE_MODE, HannahKeymapProvider, registerHannahKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerHannahKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getHannahModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getHannahModeStack(keymap).push("autocomplete")
