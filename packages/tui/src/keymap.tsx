...
export const HANNAH_AGENT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const HANNAH_AGENT_MODE_KEY = "hannah-agent.mode"

export const HannahKeymapProvider = KeymapProvider
export const useHannahKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type HannahModeStack = ReturnType<typeof createHannahModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, HannahModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createHannahModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(HANNAH_AGENT_MODE_KEY, HANNAH_AGENT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(HANNAH_AGENT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(HANNAH_AGENT_MODE_KEY, stack.at(-1)?.mode ?? HANNAH_AGENT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? HANNAH_AGENT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(HANNAH_AGENT_MODE_KEY, undefined)
...
export function useHannahModeStack() {
  return getHannahModeStack(useHannahKeymap())
}

export function getHannahModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Hannah mode stack is not registered for this keymap")
  return value
}
...
export function registerHannahKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createHannahModeStack(keymap)
