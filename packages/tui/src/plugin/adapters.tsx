import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@hannah/plugin/tui"
...
import type { useHannahKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useHannahKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getHannahModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getHannahModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
